...
typedef struct
{
	...
	double delta_ut1;    // Fractional second difference between UTC and UT which is used
						 ...
						 // where delta_ut1 = DUT1 = -0.1 seconds beginning 17 Jan 2019 at 0000 UTC
	...
	double delta_t;      // Difference between earth rotation time and terrestrial time
						 ...
						 // where delta_t = 32.184 + (TAI-UTC) - DUT1                    TAI-UTC = 37.000 000 secods
	...
	double pressure;     // Annual average local pressure [millibars]                                1018
						 // valid range:    0 to 5000 millibars,       error code: 12

	double temperature;  // Annual average local temperature [degrees Celsius]
						 // valid range: -273 to 6000 degrees Celsius, error code; 13   13 celcius

	double slope;        // Surface slope (measured from the horizontal plane)
						 // valid range: -360 to 360 degrees, error code: 14    0 degrees

	double azm_rotation; // Surface azimuth rotation (measured from south to projection of surface normal on horizontal plane, negative east) 0 degrees
